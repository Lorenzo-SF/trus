...
print_header(){ 
    ...
    print_message "    &&&     **    &   ***     &&&    ${HEADER_LOGO[1]}"
    print_message "   &&&&                      &&&&&   ${HEADER_LOGO[2]}   $HEADER_MESSAGE"
    print_message "   &&&                     &&& &&&   ${HEADER_LOGO[3]}"
    print_message "  &&&&                  &&&&&  &&&&  ${HEADER_LOGO[4]}   Usuario: $(echo "$(getent passwd $USER)" | cut -d ':' -f 5 | cut -d ',' -f 1) ($USER)"
    print_message "  &&&&         &&&&&&&&&&&     &&&&  ${HEADER_LOGO[5]}   Equipo: $HOSTNAME"
...
extract_start_option() {
    ...
    if [[ "$text" =~ ^([0-9]+)\ -(.*) ]]  ||
       ...
       [[ "$text" =~ ^(salir)\ (.+) ]]; then
        ...
        echo "No se encontró un número o parámetro válido en el texto: $text"
        return
    fi
