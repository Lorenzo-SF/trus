...
tlp_config() {
    ...
    exec_command " tlp start"
    exec_command " systemctl enable tlp.service"
...
install_trus() {
    ...
    cp $TRUS_DEFAULT_CONFIG trus.config
     ...
     rm -f $TRUS_LINK_PATH &&  ln -s $TRUS_PATH $TRUS_LINK_PATH
...
APT_INSTALLATION_PACKAGES=("curl" "zsh" "unzip" "vim" "jq" "screen" "tmux" "git" "docker.io" "postgresql-client" "xclip" "gdebi-core" "fonts-powerline" "xsltproc" "fop" "libxml2-utils" "wmctrl" "fzf" "sl" "neofetch")
        ...
        for package in "${APT_INSTALLATION_PACKAGES[@]}"; do
            ...
            exec_command " apt -qq install -y --install-recommends $package"
    ...
    if [ ! -e "/tmp/truedat_installation" ]; then
        ...
        if [ -f "$SSH_PUBLIC_FILE" ]; then
            ...
            cd $DEV_PATH
             ...
             cp elastic-search/999-map-count.conf /etc/sysctl.d/
...
install_containers() {    
    if [ ! -e "/usr/local/bin/docker-compose" ]; then        
        ...
        if [ ! -f /usr/local/bin/docker-compose ]; then
            exec_command " curl -L 'https://github.com/docker/compose/releases/download/v2.16.0/docker-compose-$(uname -s)-$(uname -m)' -o /usr/local/bin/docker-compose"
        fi

        exec_command " chmod +x /usr/local/bin/docker-compose"

        if ! getent group docker > /dev/null 2>&1; then
            exec_command " groupadd docker"
        fi
        ...
        exec_command " chmod 666 /var/run/docker.sock"

        start_containers

            print_message "Contenedores instalados y arrancados" "$COLOR_SUCCESS" "both"
    fi
}
...
install_kubectl() {
    if [ ! -e "$KUBE_PATH" ]; then
        ...
        exec_command "curl -LO https://dl.k8s.io/release/v1.23.6/bin/linux/amd64/kubectl &&  install -o root -g root -m 0755 kubectl /usr/local/bin/kubectl"
...
install_zsh() {
    ...
    print_message_with_animation "Instalando $package" "$COLOR_TERNARY"
    exec_command " apt install -y --install-recommends zsh"
    print_message "$package instalado" "$COLOR_SUCCESS"
...
start_truedat() {
    ...
    add_terminal_to_tmux_session "$(($(tmux list-panes -t truedat | awk 'END {print $1 + 0}') + 1))" "source tools 'Truedat Utils (TrUs)'; print_semiheader 'Truedat'; print_message 'Truedat está arrancado' '$COLOR_PRIMARY'; print_message 'Para acceder a la session, utiliza \"trus --attach\"' '$COLOR_SECONDARY';"
    ...
    if print_question "SE VA A PROCEDER HACER BACKUP DE LAS CLAVES '$TRUEDAT' ACTUALES, BORRAR LA CLAVE EXISTENTE Y CREAR UNA NUEVA HOMÓNIMA" "$COLOR_ERROR" = 0; then
        ...
        if [ -f "$SSH_PUBLIC_FILE" ] || [ -f "$SSH_PRIVATE_FILE" ]; then
            ...
            mkdir -p "$SSH_PATH/backup_$(date +%Y%m%d_%H%M%S)"
                    print_message "Carpeta creada: $SSH_PATH/backup_$(date +%Y%m%d_%H%M%S)" "$COLOR_TERNARY"
